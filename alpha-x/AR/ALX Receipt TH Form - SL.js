...
define(['N/file', 'N/render', 'N/record', 'N/search', 'N/config', 'N/runtime', 'N/xml'], function (file, render, record, search, config, runtime, mxml) {
  ...
  function onRequest(context) {
    try {
      ...
      var cnRecord = record.load({ type: 'customrecord_ldb_rcpt', id: context.request.parameters.recId });
      ...
      var header = '';
      var headerEng = '';
      if (custf == 'ALPHA X - Customer Payment Service') {
        // TEMPLATE = "./REC PP.xml";
        // header = 'ใบเสร็จรับเงิน';
        headerEng = 'Tax Receipt';
      } else if (custf == 'ALPHA X - Customer Payment Product') {
        // TEMPLATE = "./RECTAX PP.xml";
        // header = 'ใบเสร็จรับเงิน / ใบกำกับภาษี';
        headerEng = 'Receipt';
      }

      // if (custf == 'CHAW Product Customer Payment') {
      TEMPLATE = './ALX REC TH.xml';
      // } else {
      //   TEMPLATE = './tax rec.xml';
      // }
      ...
      var signName = companyInfo.getValue('custrecord_subsidiary_signature_name');
      var position = companyInfo.getValue('custrecord_subsidiary_position');
      var signNameTh = companyInfo.getValue('custrecord_subsidiary_sign_name_thai');
      var positionTh = companyInfo.getValue('custrecord_subsidiary_position_thai');
      var comAddressTH = companyInfo.getValue('custrecord_subsidiary_address_thai')
      ...
      companyInfo = {
        ...
        companyaddress: wordWrap(comAddress, 40),
        tel: tel,
        fax: fax,
        vatno: vatno,
        signName: signName,
        position: position,
        signNameTh: signNameTh,
        positionTh: positionTh,
        comAddressTH: wordWrap(comAddressTH, 50),
      };
      ...
      var customerId = cnRecord.getValue('custrecord_ldb_rcpt_customer');
      var customerAddress = getCustomerBillAddress(customerId);
      ...
      if (!isEmpty(customerAddress)) {
        var billName = escape_for_xml(customerAddress.getValue('addressee'));
      ...
      billAddress = wordWrap(billAddress, 100);

      var appName = getApprovedPerson(context.request.parameters.recId);

      var itemarr = getItem(cnRecord);
      var totalnetRcv = 0;
      itemarr.forEach(function(item){
        totalnetRcv += item.netRcv*1
      })

      renderer.addRecord({ templateName: 'record', record: cnRecord });

      var rec_bank_acc;
      var cus_bank_acc = cnRecord.getText("custrecord_ldb_rcpt_cus_bank_account");

      if(isEmpty(cus_bank_acc)){
        var alpha_bank_acc = cnRecord.getText("custrecord_ldb_rcpt_alpha_bank_")
        rec_bank_acc = alpha_bank_acc
      } else{
        rec_bank_acc = cus_bank_acc
      }
    ...
    if (!isEmptyObj(custData)) {
      var custisperson = custData.isperson ;
    }
      ...
      var data = {
        ...
        itemarr: itemarr,
        totalnetRcv:totalnetRcv,
        custisperson: custisperson,
        rec_bank_acc: rec_bank_acc,
      };
  ...
  function getItem(recordObj) {
    var linecount = recordObj.getLineCount({
      sublistId: 'recmachcustrecord_lds_rcpt_trans',
    });
    var itemArr = [];
    for (var i = 0; i < linecount; i++) {
      var principle = recordObj.getSublistValue({
        sublistId: 'recmachcustrecord_lds_rcpt_trans',
        fieldId: 'custrecord_ldb_rcpt_principal_trans_dt',
        line: i,
      });
      var interest = recordObj.getSublistValue({
        sublistId: 'recmachcustrecord_lds_rcpt_trans',
        fieldId: 'custrecord_ldb_rcpt_interest_trans_dt',
        line: i,
      });
      var fee = recordObj.getSublistValue({
        sublistId: 'recmachcustrecord_lds_rcpt_trans',
        fieldId: 'custrecord_ldb_rcpt_fee_trans_dt',
        line: i,
      });
      var netRcv = recordObj.getSublistValue({
        sublistId: 'recmachcustrecord_lds_rcpt_trans',
        fieldId: 'custrecord_ldb_rcpt_net_trans_dt',
        line: i,
      });
      var interestRate = recordObj.getSublistValue({
        sublistId: 'recmachcustrecord_lds_rcpt_trans',
        fieldId: 'custrecord_ldb_rcpt_interate_trans_dt',
        line: i,
      });

      if(isEmpty(principle)){
        principle = '-'
        };
