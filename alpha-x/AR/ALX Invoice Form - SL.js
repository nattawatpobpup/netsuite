...
define(['N/file', 'N/render', 'N/record', 'N/search', 'N/config', 'N/runtime', 'N/xml'], function (file, render, record, search, config, runtime, mxml) {
  ...
  function onRequest(context) {
    try {
      ...
      var recordObj = record.load({ type: 'customrecord_lending_business_inv', id: context.request.parameters.recId });
      ...
      var checkABA = recordObj.getValue('custrecord_ldb_inv_alpha_bank_account')
      // log.debug('checkCBA',checkCBA)
      if (!isEmpty(checkABA)) {
             TEMPLATE = './ALX Invoice ABA.xml';
      } else {
 
        TEMPLATE = './ALX Invoice CBA.xml';
      }
      ...
      var customrecord_lending_business_invSearchObj = search.create({
        type: 'customrecord_lending_business_inv',
        filters: [['internalid', 'anyof', context.request.parameters.recId]],
        columns: [
          search.createColumn({
            name: 'custrecord_account_ldb_trans_dt',
            join: 'CUSTRECORD_LDB_TRANS',
            label: 'AccountNo',
          }),
          search.createColumn({
            name: 'custrecord_ldb_period_from_trans_dt',
            join: 'CUSTRECORD_LDB_TRANS',
            label: 'PeriodFrom',
          }),
          search.createColumn({
            name: 'custrecord_ldb_period_to_trans_dt',
            join: 'CUSTRECORD_LDB_TRANS',
            label: 'PeriodTo',
          }),
          search.createColumn({
            name: 'custrecord_ldb_principal_trans_dt',
            join: 'CUSTRECORD_LDB_TRANS',
            label: 'Principal',
          }),
          search.createColumn({
            name: 'custrecord_ldb_interest_trans_dt',
            join: 'CUSTRECORD_LDB_TRANS',
            label: 'Interest',
          }),
          search.createColumn({
            name: 'custrecord_ldb_fee_trans_dt',
            join: 'CUSTRECORD_LDB_TRANS',
            label: 'Fee',
          }),
          search.createColumn({
            name: 'custrecord_ldb_total_amount__trans_dt',
            join: 'CUSTRECORD_LDB_TRANS',
            label: 'TotalAmount',
          }),
        ],
      ...
      var searchColumns = customrecord_lending_business_invSearchObj.columns;
      customrecord_lending_business_invSearchObj.run().each(function (result) {
        var accountLine = {};

        for (var i = 0; i < searchColumns.length; i++) accountLine[searchColumns[i].label] = result.getValue(searchColumns[i]);
        // .run().each has a limit of 4,000 results

        
        
          
        totalAmount = accountLine.TotalAmount;
        accountArr.push({
          accountno: accountLine.AccountNo,
          periodFrom: accountLine.PeriodFrom,
          periodTo: accountLine.PeriodTo,
          principal: accountLine.Principal,
          interest: accountLine.Interest,
          fee: accountLine.Fee,
          totalAmount: accountLine.TotalAmount,
        });

        if(accountLine.Principal == 0){
          accountLine.Principal = '-'
        };
